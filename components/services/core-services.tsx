        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {coreServices.map((service, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 hover:shadow-xl hover:border-orange/30 transition-all duration-300 h-full flex flex-col"
            >
              <div className="mb-4 bg-navy/5 w-16 h-16 rounded-full flex items-center justify-center">
                {service.icon}
              </div>
              <h3 className="text-xl font-bold text-navy mb-2">{service.title}</h3>
              <p className="text-gray-600 flex-grow">{service.description}</p>
              <div className="mt-4 pt-4 border-t border-gray-100">
                <a href={`#${service.id}`} className="text-orange font-medium hover:text-orange-600 transition-colors flex items-center">
                  Learn more
